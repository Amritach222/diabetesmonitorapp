...
// mail option object
   mailOptions = {
    ...
    to: emails,
    subject: 'Reminder to upload your activities !',
    html:'<!DOCTYPE html>\n' +
...
cron.schedule('00 58 09 * * *', () => {
  mysql.query(`SELECT * FROM usersignup WHERE haveDiabetes=?`, ['Yes'], (err, result, fields) => {
    if (err) {
      console.log(err)
    } else {
      result.map((item) => {
        insulin_emails.push(item.email)
      })
  ...
  insulinMessage = {
    from: 'sugartrackers@gmail.com',
    to: insulin_emails,
    subject: 'Reminder to take insulin !',
    html:'<!DOCTYPE html>\n' +
      '<html lang="en" xmlns="http://www.w3.org/1999/xhtml" xmlns:o="urn:schemas-microsoft-com:office:office">\n' +
      '<head>\n' +
      '  <meta charset="UTF-8">\n' +
